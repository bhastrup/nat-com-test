...
class SOAPProjector:
    ...
    def __init__(self, mol_dataset: str = "qm7", data_dir: Path = None, formulas: List[str] = None):
        ...
        if formulas:
            ...
            self.ref_data.smiles = {
                formula: smiles for formula, smiles in self.ref_data.smiles.items() if formula in formulas
            }
        ...
        self.embedder = SOAP(
            species=species, periodic=False, r_cut=rcut, n_max=nmax, l_max=lmax, average="outer", sparse=False
        )
    ...
    def _embed(self, atoms_objects: List[Atoms]) -> np.ndarray:
        return self.embedder.create(system=atoms_objects, centers=None, n_jobs=1, verbose=False)
    ...
    def __call__(
        self, smiles_set: Set[str], find_complement_set: bool = False, use_fitted_projector: bool = True
    ) -> np.ndarray:
    ...
    def _dataframe_to_observations(self, df: pd.DataFrame) -> np.ndarray:
        ...
        for i in range(len(pos)):
            ...
            obs = self.embedder.observation_space.build(
                atoms=atoms, formula=((z, 0) for z in self.embedder.observation_space.bag_space.zs)
            )
