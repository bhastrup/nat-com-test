...
class PPOBufferContainerDeploy(PPOBufferContainer):
    ...
    def store(
        ...
        for i, i_original in enumerate(self.active_buffers):
            ...
            if terminals[i]:
                episodic_ret, episode_length = self.buffers[i_original].finish_path(0.0)
                assert episodic_ret is not None and episode_length > 0
                self.episodic_returns.append(episodic_ret)
                self.episode_lengths.append(episode_length)

                self.ep_ret_unmixed[i_original].append(episodic_ret)
