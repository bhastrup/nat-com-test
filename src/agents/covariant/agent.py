...
from src.agents.covariant import tools, so3_tools
from src.agents.covariant.gmm import GaussianMixtureModel
from src.agents.covariant.modules import Cormorant, CormorantMixer
from src.agents.covariant.so3_tools import AtomicScalars
from src.agents.covariant.spherical_dists import SphericalDistribution, ExpSO3Distribution, SO3Distribution, get_grid
from src.agents.modules import MLP, masked_softmax, to_one_hot
from src.rl.spaces import ObservationType, ActionType, ObservationSpace, ActionSpace
...
class CovariantAC(AbstractActorCritic):
    def __init__(
        ...
        hydrogen_delay: bool = False,
        no_hydrogen_focus: bool = False,
        device=None,
    ):
    ...
    def step(self, observations: List[ObservationType], actions: Optional[np.ndarray] = None) -> dict:
        ...
        focus_probs = masked_softmax(focus_logits, mask=focus_mask_choose)  # (batches, atoms)
        ...
        element_probs = masked_softmax(element_logits, mask=element_mask)  # (batches, zs)
