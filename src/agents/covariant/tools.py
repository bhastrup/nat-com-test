...
def atoms_to_feats(atoms: Atoms, dtype: torch.dtype, device: torch.device) -> Dict[str, torch.Tensor]:
    return {
        ...
        "charges": torch.tensor(
            [ase.data.atomic_numbers[atom.symbol] for atom in atoms], dtype=torch.int, device=device
        ),
...
def process_atoms_list(
    atoms_list: List[Atoms], max_num_atoms: int, dtype: torch.dtype, device: torch.device
